...
このブランチは初期設計時の master から挙動が変わっている (検証強化、`Wareki::InvalidDate` の導入、`ERA_JD_LOOKUP (superseded: Task 6 のゴールデンテストでこの事前計算方式の南朝優先仮定が Ruby と不一致と判明し、Ruby find_era 忠実な逆順線形走査に置換済み — このスニペットを再利用しないこと)` による era 解決の再設計、北朝元号の索引上の扱い変更)。
起草時の HEAD は cdfa641 (その後 6459443 に再ピン。差分は %% エスケープ強化のみ) だが、ブランチは進行中の可能性があるため、**各計画の実行開始時に HEAD を再確認**し、進んでいた場合は `lib/` 配下の差分を評価してから着手する。
データ生成・ゴールデン生成ツールは、生成物ヘッダに `git -C ../wareki describe --always --dirty` の値を記録する。

### 南北朝期の元号解決に関する裁定 (2026-07-13、ユーザー確定)

**更新 (同日)**: Ruby 側で PR #23 (fix/2026-07-13-review) が master にマージされた後、裁定どおり era 解決のみ北朝優先へ戻す **fix/north-court-era-priority** ブランチ (a37fe90) が作られた。以降の移植対象・照合基準はこのブランチとする (review ブランチの全改善 + 北朝優先 = 本裁定の Ruby 側実装)。ゴールデンフォーマットデータはこのブランチから生成済み。


wareki の master と fix/2026-07-13-review は南北朝期 (1331-1394) の JD→元号解決が異なる (master = 逆順線形走査で北朝が勝つ、review = ERA_JD_LOOKUP で南朝優先)。
ユーザー裁定: **北朝側が正しい** (Ruby 側は今後さらに変更予定)。パースは南北両方の元号名を受け入れ、JD からの解決 (出力) は北朝になる。南朝の元号は JD 解決では返らない。
したがって移植は「review ブランチの挙動 (検証強化、InvalidDate、%% エスケープ) + era 解決のみ master 挙動」を対象とする。現在の JS 実装 (Task 6 修正後) はこの裁定と一致しており、ゴールデンデータの再生成は不要。
Plan 2 のパーサ・フォーマッタ移植で review ブランチのコードを参照する場合、wareki の作業ツリーは master に切り替わっているため `git -C ../wareki archive fix/2026-07-13-review | tar -x -C <tmpdir>` で展開したコードに対して実行・照合すること (era 解決の期待値のみ master / 現行 JS に合わせる)。
...
- `engines: { "node": ">=22" }`。ビルドは tsdown、テストは Vitest、公開前検証に publint + @arethetypeswrong/cli。(tsdown 0.22 系が Node ^22.18 || >=24.11 を要求するため、Node フロアは 20 ではなく 22 とする。ユーザー確認済み。)
...
export class WarekiInvalidDateError extends WarekiParseError {} // Ruby: Wareki::InvalidDate < ArgumentError
...
- `WarekiInvalidDateError` は `WarekiParseError` を継承する (Ruby の `Wareki::InvalidDate < ArgumentError` と同じ関係)。和暦の構文としては認識できたが日付として不成立 (存在しない月日、月の範囲外など) な場合にのみ投げ、構文自体が解釈不能な場合は従来どおり `WarekiParseError` を投げる。`parseToDate` は Ruby の `rescue InvalidDate; raise` / `rescue ArgumentError, UnsupportedDateRange` の分岐を再現し、`WarekiInvalidDateError` の場合は `new Date(str)` へフォールバックせず即座に再 throw する (他の `WarekiParseError` / `UnsupportedDateRangeError` は従来どおりフォールバックする)。これにより `平成五年二月三十日 2020-01-02` のような「存在しない和暦日付 + フォールバック可能な文字列」の入力で、誤った日付を静かに返すことを防ぐ。
...
- GitHub Actions: Node 22/24 のマトリクスで build + test + publint + attw。
...
## 追補 (2026-07-13): 上流同期 — 北朝マージと時刻の漢数字対応

- **北朝優先が上流 (wareki) `origin/master` にマージ済み (PR #24)**。設計時点で参照していた
  `fix/north-court-era-priority` (a37fe90) ブランチは不要になった。JD→元号解決の北朝優先は
  現行 JS 実装・ゴールデンデータと一致しており、移植・照合の基準は clean な `origin/master` でよい。
  リリース手順のデータ再生成 (`docs/RELEASING.md` 手順2) も専用ブランチ不要に簡素化済み。
- **時刻の漢数字対応を移植 (上流 PR #26 "time kansuji support")**。本ライブラリはビルトイン非拡張の
  方針のため、Ruby の `Date.parse` / `Time.parse` / `Time#strftime` 上書きは既存の公開サーフェスに統合した。
  - 新モジュール `src/time.ts`: `normalizeTime(str)` (Ruby `Utils.normalize_time` + `TIME_REGEX` の移植。
    正午→12:00、午前/午後、半→:30、範囲チェックなし、最初の1箇所のみ置換)、
    `formatTime(time, fmt)` (Ruby `Utils.expand_time_format`。`%JT` 系ディレクティブ Tf/TF/TH/THk/TM/TMk/TS/TSk)。
    `%%` エスケープ機構と `number_format` は `src/format.ts` から `expandJDirectives` / `fmtNum` として
    抽出・共有し、重複させていない。
  - 統合: 上位 `format(date, fmt)` は **JS `Date`** 入力時のみ `%JT` をローカル時刻から展開してから `%J` 日付を展開する
    (Ruby `expand_all_wareki_formats` = 時刻→日付の順)。**`WarekiDate`** 入力時は時刻を持たないため `%JT` はリテラル維持。
    `parseToDate(str)` は `normalizeTime` を先に適用し、和暦日付が解釈できて時刻表記があればローカル時刻としてセットする。
    範囲外時刻 (Ruby `Time.parse` の受理範囲: 時≤24 かつ 24 は 24:00:00 のみ、分≤59、秒≤60 を超えるもの) は `WarekiParseError`。
  - 新規エクスポート: `normalizeTime`, `formatTime`, 型 `TimeParts`。
  - 設計差 (Ruby との既知の違いに追加): (a) `parseToDate` は日付を含まない裸の時刻 (例 `午後三時半`) に対して
    Ruby `Time.parse` の「本日＋時刻」挙動を**再現しない** (フォールバックの `new Date()` が解釈できず `WarekiParseError`)。
    これは Ruby `Date.parse('12時34分')` → `ArgumentError` の spec に一致する。(b) 標準 strftime 委譲が無いため
    `%H`/`%M`/`%S` は Date 入力でも未変換で通過する (既存の意図的差異。時刻を出すには `%JT` 系を使う)。
